            ...
            {courses.map((program) => (
              <Link to={`/${program.slug}`} key={program.id} className="bg-white rounded-2xl shadow-sm border border-stone-200 hover:shadow-md transition overflow-hidden group flex flex-col cursor-pointer">
                ...
                <div className="p-6 md:p-8 flex-1">
                  ...
                  <span className="inline-block mt-4 text-amber-800 font-semibold group-hover:text-amber-950 transition-colors">İncele &rarr;</span>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-stone-200/50 p-8 md:p-12 border border-stone-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="home_studentName" className="block text-sm font-medium text-stone-700 mb-2">Öğrenci Adı *</label>
                  <input id="home_studentName" required type="text" name="studentName" value={formData.studentName} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl border border-stone-300 focus:ring-2 focus:ring-amber-800/20 focus:border-amber-800 outline-none transition" />
                </div>
                <div>
                  <label htmlFor="home_studentSurname" className="block text-sm font-medium text-stone-700 mb-2">Öğrenci Soyadı *</label>
                  <input id="home_studentSurname" required type="text" name="studentSurname" value={formData.studentSurname} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl border border-stone-300 focus:ring-2 focus:ring-amber-800/20 focus:border-amber-800 outline-none transition" />
                </div>
                <div>
                  <label htmlFor="home_parentName" className="block text-sm font-medium text-stone-700 mb-2">Veli Adı (İsteğe Bağlı)</label>
                  <input id="home_parentName" type="text" name="parentName" value={formData.parentName} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl border border-stone-300 focus:ring-2 focus:ring-amber-800/20 focus:border-amber-800 outline-none transition" />
                </div>
                <div>
                  <label htmlFor="home_parentSurname" className="block text-sm font-medium text-stone-700 mb-2">Veli Soyadı (İsteğe Bağlı)</label>
                ...
                <div className="md:col-span-2">
                  ...
                  <input id="home_branch" required type="text" name="branch" value={formData.branch} onChange={handleInputChange} placeholder="Örn: Sportif Latin, Şan Dersi..." className="w-full px-4 py-3 rounded-xl border border-stone-300 focus:ring-2 focus:ring-amber-800/20 focus:border-amber-800 outline-none transition" />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full bg-[#128C7E] text-white py-4 rounded-xl text-lg font-bold hover:bg-[#075E54] transition shadow-lg shadow-[#128C7E]/20 disabled:opacity-70 disabled:cursor-not-allowed mt-4 flex items-center justify-center gap-3"
              >
